         ...
         if (strpos(get_post_meta($product->get_id(), 'scd_other_options')[0]["currencyVal"],scd_get_target_currency()) == 0 || strpos(get_post_meta($product->get_id(), 'scd_other_options')[0]["currencyVal"],scd_get_target_currency()) != false) {
             ...
             foreach($prices as $price){
                if(strpos($price,scd_get_target_currency())){
					...
					//if(explode('_',$each_prices[1])[2]!=NULL){
    ...
    /*if($product->get_children()){
        ...
        foreach ($product->get_children() as $variation_id) {
            ...
            if(get_post_meta($variation_id, 'scd_other_options')){
                if (strpos(get_post_meta($variation_id, 'scd_other_options')[0]["currencyVal"],scd_get_target_currency()) != FALSE) {
                    $prices = explode(",", substr(get_post_meta($variation_id, 'scd_other_options')[0]["currencyPrice"], 0, -1));
                    if(sizeof($prices)>1){
                        foreach($prices as $price){
                            if(strpos($price,scd_get_target_currency())){
                                $each_prices = explode('-',$price);
                                if(explode('_',$each_prices[1])[2]!=""){
                                    array_push($array_price,explode('_',$each_prices[1])[2]);
                                }else{ 
                                    array_push($array_price,explode('_',$each_prices[0])[2]);
                                }
                            }   
                        }
                    }else{
                        $regprice = get_post_meta($variation_id, '_meta_regular_price', TRUE);
                        $saleprice = get_post_meta($variation_id, '_meta_sale_price', TRUE);
                        if(($regprice > 0)){
                            if($saleprice==""){
                                array_push($array_price,$regprice);
                            }else{
        ...
        if(sizeof($array_price) > 0){
            ...
            $price2 = $array_price[sizeof($array_price)-1];    
            if($price1==$price2){
                $price_html = get_woocommerce_currency_symbol(scd_get_target_currency()).''.$price1;
            }else{
                $price_html = "<ins>".get_woocommerce_currency_symbol(scd_get_target_currency()).$price1."</ins> - <ins>".get_woocommerce_currency_symbol(scd_get_target_currency()).$price2."</ins>";
            }
        }
    }*/
    if($price_html!=""){
...
// function return_custom_regular_price($price, $product)
// {

//     $new_price = (get_post_meta($product->get_id(), '_meta_regular_price', TRUE));
//     $old_price = (get_post_meta($product->get_id(), '_regular_price', TRUE));
//     $rate = scd_get_conversion_rate(scd_get_target_currency(), get_woocommerce_currency());
//     if ($new_price != "") {
//         return $new_price;
//     }elseif($old_price!=""){
//         return floatval($old_price * $rate);
//     }
//     return $price;
// }

// function return_custom_sale_price($price, $product)
// {

//     $new_price = (get_post_meta($product->get_id(), '_meta_sale_price', TRUE));
//     $old_price = (get_post_meta($product->get_id(), '_sale_price', TRUE));
//     $rate = scd_get_conversion_rate(scd_get_target_currency(), get_woocommerce_currency());
//     if ($new_price != "") {
//         return $new_price;
//     }elseif($old_price!=""){
...
function return_custom_price($price, $product)
{
    ...
    $new_sale_price = (get_post_meta($product->get_id(), '_meta_sale_price', TRUE));
    $old_sale_price = (get_post_meta($product->get_id(), '_sale_price', TRUE));
    ...
    if($new_price!=""){
        ...
        if ($old_sale_price!="") {
            return floatval($old_sale_price * $rate);
        } else {
            return floatval($old_price * $rate);
        } 
    }
...
// add_filter('woocommerce_get_regular_price', 'return_custom_regular_price', 10, 2);
// add_filter('woocommerce_get_sale_price', 'return_custom_sale_price', 10, 2);
